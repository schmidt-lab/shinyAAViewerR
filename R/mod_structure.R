...
mod_structure_ui <- function(id) {
  ...
  tagList(
    menuItem("structure",
      ...
      selectInput(ns('structureColorScale'), 'Scale', c(
        'RdYlBu', 'rwb', 'rainbow', 'BrBG', 'PiYG', 'PuOr', 'PRGn', 'PuBuGn', 'RdGy', 'Viridis', 'YlGnBu', 'YlOrRd'
      ), selected = 'rwb'),
      colourpicker::colourInput(ns("structureColor"), label = NULL, "grey80", palette = "limited", closeOnClick = TRUE),
...
observeEvent(r$sidebarItemExpanded, {
  if (r$structure$loaded == FALSE && (r$sidebarItemExpanded == "structure")) { # reset in mod_NGLVieweROutput
    if (!is.null(r$fileInput$structure)) {
      ...
      updateSelectInput(session, "structureColorScale", selected = data$colorScale)
      ...
      updateSelectInput(session, "structureColorScale", selected = "rwb")
  ...
  observe({
    ...
    Viewer_proxy %>% addSelection(input$structureType,
      param = list(
        ...
        colorScale = input$structureColorScale,
